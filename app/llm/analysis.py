...
Reference: ai-clipper-id/sosmed/llm/analysis.py
"""
...
from app.llm.backends import call_llm
from app.utils import MAX_CLIPS_HARD_LIMIT, SYSTEM_PROMPT, log
...
def _normalize_score_fields(c: dict[str, Any]) -> dict[str, int]:
    ...
    if (
        hook == 0
        and insight_density == 0
        and retention == 0
        and emotional_payoff == 0
        and clarity == 0
    ):
        # Map legacy fields to new ones
        hook = _to_score(
            c.get("score_newsworthy", c.get("score_shareability", 0))
        )  # newsworthy acts as hook
...
def _validate_clips(
    ...
    for c in scored_clips:
        ...
        if dur < min_dur or dur > max_dur:
            log(
                "DEBUG",
                f"Skip '{title}' [{s:.0f}-{e:.0f}]: duration {dur:.0f}s "
                f"outside {min_dur}-{max_dur}s",
            )
            continue
        if video_duration and e > video_duration + 2:
            log(
                "DEBUG",
                f"Skip '{title}' [{s:.0f}-{e:.0f}]: end exceeds video "
        ...
        if score < min_score:
            log(
                "DEBUG",
                f"Skip '{title}' [{s:.0f}-{e:.0f}]: score {score:.1f} < {min_score}",
            )
            continue
        # Normalize score fields for output
        # (no hard hook floor — clip_score weighting handles quality).
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            if n_chunks > 1:
                chunk_info = (
                    ...
                    f"setiap subtopik yang layak harus jadi klip."
                )
                log(
                    "INFO",
                    f"Chunk {idx}/{n_chunks}: {chunk_start:.0f}s → {chunk_end:.0f}s "
