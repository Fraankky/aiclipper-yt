...
Reference: ai-clipper-id/sosmed/llm/fix_clips.py
"""
...
def _translate_to_indonesian(
    ...
    if result and isinstance(result, list):
        ...
        if all(isinstance(c, dict) for c in result):
            ...
            result_map = {
                (round(c.get("start", 0), 2), round(c.get("end", 0), 2)): c for c in result
            }
            matched_count = 0
...
def _fix_caption_topic_mismatch(
    ...
    system_message = (
        "You are an expert content strategist ensuring social media content consistency."
    )
    result = call_llm(system_message, user_message, api_key, llm_model)
    ...
    if result and isinstance(result, list):
        if all(isinstance(c, dict) for c in result):
            ...
            for improved_clip in result:
                key = (
                    round(improved_clip.get("start", 0), 2),
                    round(improved_clip.get("end", 0), 2),
                )
                if key in orig_map:
                    orig = orig_map[key]
...
def translate_subtitle_words(
    ...
    if not result or not isinstance(result, list) or not all(isinstance(p, dict) for p in result):
        raise RuntimeError(f"Subtitle translation failed: LLM returned {result!r}")
    ...
    for i, grp in enumerate(groups):
        ...
        if not trans_words:
            raise RuntimeError(f"Subtitle translation produced empty phrase for id={i}")
    ...
    log(
        "OK",
        f"Subtitle translation: {len(words)} original words → {len(translated_words)} translated words",
    )
