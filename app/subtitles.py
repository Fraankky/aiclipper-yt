...
def _group_words(
    ...
    for word in words:
        if current:
            ...
            if len(current) >= max_words or group_dur > max_duration or gap > max_gap:
...
def generate_ass_subtitles(
    ...
    for group, (g_start, g_end) in zip(groups, clamped, strict=True):
        ...
        for i, word in enumerate(group):
            effective_start = g_start if i == 0 else group[i - 1]["end"]
...
def generate_title_overlay(
    ...
    font_size = int(play_res_x * 0.09) if aspect < 1.0 else int(play_res_y * 0.10)
    ...
    fade_out_dur = max(100, int(duration * 200)) if duration < 1.5 else int(0.4 * 1000)
