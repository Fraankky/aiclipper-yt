"""
Utils module — logging, ANSI colors, constants, ffmpeg detection,
clip boundary helpers, internal field cache.

Reference: ai-clipper-id/sosmed/utils.py
"""
...
def _find_ffmpeg_with_libx264() -> tuple[str, str]:
    ...
    checks the def]]ault ``ffmpeg`` first; if it lacks libx264, it tries the
    ...
    for ffmpeg_bin in candidates:
        try:
            ...
            if "libx264" in (result.stdout + result.stderr):
                ...
                ffprobe_bin = (
                    str(Path(ffmpeg_bin).parent / "ffprobe") if "/" in ffmpeg_bin else "ffprobe"
                )
                if "/" in ffmpeg_bin:
