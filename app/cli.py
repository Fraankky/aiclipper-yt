...
from app.postprocess import postprocess_clips
from app.prefilter import prefilter_segments
from app.subtitles import get_clip_words
...
def _should_translate_to_indonesian(detected_language: dict) -> bool:
    """Determine if subtitle translation to Indonesian is needed."""
...
def main() -> None:
    ...
    ap.add_argument(
        "--lang",
        default="id",
        help="Language code — 'id' Indonesian, 'en' English, or None for auto-detect (default: id)",
    )
    ap.add_argument(
        "--min", type=int, default=15, help="Min clip duration in seconds (default: 15)"
    )
    ap.add_argument(
        "--max", type=int, default=180, help="Max clip duration in seconds (default: 180)"
    )
    ap.add_argument(
        "--max-clips",
        type=int,
        default=MAX_CLIPS_HARD_LIMIT,
    ...
    ap.add_argument("--no-vad", action="store_true", help="Disable VAD filtering for transcription")
    ap.add_argument(
        "--vad-min-silence",
        type=int,
        default=400,
    ...
    ap.add_argument(
        "--vad-speech-pad", type=int, default=200, help="VAD speech padding in ms (default: 200)"
    )
    ap.add_argument(
        "--batch", type=int, default=16, help="Whisper batch size (default: 16; lower if OOM)"
    )
    ap.add_argument(
        "--workers",
        type=int,
    ...
    ap.add_argument("--output", default="clips", help="Output directory (default: ./clips)")
    ...
    ap.add_argument(
        ...
        default=True,
        help="TikTok-style word-by-word subtitles (default: on, --no-subtitles to disable)",
    )
    ap.add_argument(
        "--subtitle-position",
        default="lower",
    ...
    ap.add_argument(
        "--example-count", type=int, default=3, help="Number of example clips to use (default: 3)"
    )
    ap.add_argument(
        "--clips-file",
        default="clips/clips.json",
        metavar="FILE",
    ...
    if args.example:
        ...
        print(f"\n{BOLD}{CYAN}{'═' * 50}")
        print("   AI Video Clipper — Example Mode (Testing)")
        print(f"{'═' * 50}{RESET}")
        ...
        if pp_features:
            ...
            print("  Features  : Raw clips only")
        print()

        # Summary table
        print(
            f"{BOLD}{'#':<4} {'Score':<6} {'H/I/R/E/C':<18} {'Start':>7} {'End':>7} {'Dur':>5}  Topic{RESET}"
        )
        print("─" * 90)
        for c in clips:
            d = c["end"] - c["start"]
    ...
    print(f"\n{BOLD}{CYAN}{'═' * 50}")
    print("   AI Video Clipper — Indonesian-optimized")
    print(f"{'═' * 50}{RESET}")
    print(f"  Video     : {video.name}")
    ...
    print(
        f"  {stats['original']} → {stats['after_filter']} after filter "
    ...
    if not clips_from_cache:
        ...
        if time_saved > 1:
            log(
                "OK",
                f"Removed {time_saved:.1f}s of gaps/filler (avg {time_saved / len(clips):.1f}s per clip)",
            )
        else:
