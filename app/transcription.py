...
def transcribe(
    ...
    log(
        "INFO",
        f"Loading whisper {BOLD}{model_size}{RESET} on {device.upper()} "
        f"({compute_type}) {cache_status}",
    )
    t0 = time.time()
    model = WhisperModel(
        model_size,
    ...
    condition_prev = not model_size.startswith("distil-")
    ...
    log("OK", f"{duration:.0f}s audio → {len(segments)} segments | RTF {rtf:.2f}x | {elapsed:.1f}s")
    language_info = {
